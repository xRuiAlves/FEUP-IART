...
In this project, all of the applicable algorithms that were proposed were implemented and studied (except for the Uniform-Cost search, which explores the states in the exact same order that Breadth-First search does for this specific problem and was, therefore, not necessary for the best solution study, and for the Bi-Directional algorithms, which were not possible to be applied since the final objective state is not known \textit{a priori}).
...
In order to study the upsides and downsides of each of the algorithms, a thorough examination of their performance using different metrics (solution length, execution time and number of expanded states). When analysing the produced solutions, it was concluded that they matched with the theoretical expectations:
...
\begin{itemize}
    ...
    \item Considering the execution time required to reach a solution, the \textbf{DFS} and \textbf{Greedy} algorithms achieved solution in short execution times (although not optimal). The \textbf{Iterative Deepening} algorithm originated the longest execution times for most of the studied puzzles.
...
All of the objectives were successfully completed and future improvements could include the studying of the different heuristics in this problem and the exploration of the studied algorithms in puzzles of bigger sizes.
