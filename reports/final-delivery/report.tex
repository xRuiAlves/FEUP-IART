...
The algorithm analyzes all states with a depth inferior to N prior to analyzing states with a depth of N, to acomplish this task all nodes that need to be analyzed are kept in a queue. The next state to be analyzed is in the front of the queue and the calculated neighbours are appended to the end of this structure.

This approach has exponential complexity in both time and space, this makes the algorithm consume copius amounts of memory, making it hard to run in puzzles that have a solution size bigger than 10. However, using a set of searched states to avoid the expansion of nodes already visited, the algorithm can run in a reasonable amount of time, due to the small search space and a high frequency of repeated states. 
